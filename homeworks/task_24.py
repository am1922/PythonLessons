...
def maxBerryCount(berryCounts):
    maxСount = 0
    for i in range(len(berryCounts)):
        currentСount = berryCounts[i] + berryCounts[(i+1) % len(berryCounts)] + berryCounts[(i-1) % len(berryCounts)]
        maxСount = max(maxСount, currentСount)
    return maxСount

N = int(input("Введите количество кустов на грядке: "))
berryCounts = [random.randrange(100, 901, 50) for _ in range(N)]
print(berryCounts)
maxСount = maxBerryCount(berryCounts)
print(maxСount)